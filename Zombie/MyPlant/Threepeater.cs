...
namespace ZombiesVsPlants.MyPlant
{
    ...
    {
        public Threepeater(Street s, Floor f)
            : base(s, f)
        {
            Hp = 2147483647;
            Dir = Direction.RIGHT;
        ...
        private bool hasEnemy()
        {
            ...
            {
                //判断植物是否攻击
                if (((Zombie)Map.Zombies[j]).Floor == this.Floor || ((Zombie)Map.Zombies[j]).Street == this.Street)
                {
                    return true;
                }
        ...
        {
            //更新图片
            Bullet bullet = new PeaBullet(this.Street, this.Floor, Direction.UP);
            Map.addBullet(bullet);
        }
        public void Attack2()
        {
