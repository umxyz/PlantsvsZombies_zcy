...
namespace ZombiesVsPlants.MyMission
{
    class Mission2 : Mission
    {
        ...
        public override void initMission(Map map)
        {
            ...
            ZombiesVsPlants.PlansTime.SunFlower = ZombiesVsPlants.PlansTime.SunShroom = ZombiesVsPlants.PlansTime.TwinSunflower = 120;
            //初始化剩余界面信息
            LevelStart();
        }
        ...
        {
            //初始化背景图片
            Map.BackgroundImage = Resources.NormalBackground2();
        }
        ...
        {
            Map.initCard(new PlantCard("PuffShroom")); 
            Map.initCard(new PlantCard("SunShroom"));
            Map.initCard(new PlantCard("FumeShroom"));
            Map.initCard(new PlantCard("GloomShroom"));
            Map.initCard(new PlantCard("WallNut"));
            Map.initCard(new PlantCard("PumpkinHead"));
            Map.initCard(new PlantCard("CoffeeBean"));
            Map.initCard(new PlantCard("DoomShroom"));
        }
        ...
        public override void CreateZombies()
        {
            ...
            while (Controller.gameStatus != GameStatus.OVER && validNum == Controller.GameTime)
            {
                if (Controller.gameStatus == GameStatus.START)
                {
                    ...
                    if (ZombiesVsPlants.AttackTime.sleeptime > 500) ZombiesVsPlants.AttackTime.sleeptime -= 500; //else ZombiesVsPlants.AttackTime.sleeptime = 100;

                    if (ZombiesVsPlants.AttackTime.sleeptime < 10000)
                    {
