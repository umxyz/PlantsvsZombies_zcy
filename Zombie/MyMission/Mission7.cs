        ...
        public override void CreateZombies()
        {
            ...
            while (Controller.gameStatus != GameStatus.OVER && validNum == Controller.GameTime)
            {
                if (Controller.gameStatus == GameStatus.START)
                {
                    ...
                    if (ZombiesVsPlants.AttackTime.sleeptime > 500) ZombiesVsPlants.AttackTime.sleeptime -= 300; //else ZombiesVsPlants.AttackTime.sleeptime = 100;

                    if (ZombiesVsPlants.AttackTime.sleeptime < 200)
                    {
